...
1. **File Upload:** A .csv file containing raw sales data is uploaded to a designated AWS S3 bucket (`source-data-bucket`).
2. **Event Trigger:** The S3 PutObject event automatically triggers an AWS Lambda function.
3. **ETL Execution:** The Lambda function, written in Python, executes the core ETL logic:
...
4. **Data Warehouse:** The structured data is now available in the PostgreSQL data warehouse for querying by BI tools like Tableau or Power BI.
...
| AWS Lambda   | Serverless compute for running the ETL script.               |
| PostgreSQL   | Relational database for analytics.                           |
| Python       | Core programming language for ETL logic.                     |
| Pandas       | Data manipulation and transformation.                        |
| Boto3        | AWS SDK for Python, used to interact with S3.                |
| Psycopg2     | PostgreSQL adapter for Python.                               |
| AWS SAM      | Framework for deploying serverless infrastructure as code.   |
